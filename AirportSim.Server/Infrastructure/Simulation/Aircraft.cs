...
public class Aircraft
{
    ...
    public Aircraft(FlightEvent flightEvent, RunwayId assignedRunway)
    {
    ...
    public void Tick(double simDeltaMs, RunwayController runway, GateManager gates,
                     WeatherCondition currentWeather, int rvrMeters, int assignedHoldAltitude = 0)
    {
        ...
        {
            if (State.Phase is not (AircraftPhase.Parked or AircraftPhase.Holding or AircraftPhase.Pushback))
            {
        ...
        {
            runway.Release(State.AssignedRunway, State.FlightId);
            State.Phase     = AircraftPhase.GoAround;
            if (State.Status is not (AircraftStatus.Emergency or AircraftStatus.Diverting))
        ...
        {
            if (!runway.TryOccupy(State.AssignedRunway, State.FlightId, State.FlightType)) return;
        }
    ...
    private void TickTurnaround(double simDeltaMs)
    {
        ...
        {
            AircraftType.Small  => 30.0,
            AircraftType.Medium => 45.0,
            AircraftType.Large  => 90.0,
            _                   => 45.0
        };

        double phaseMultiplier = State.Turnaround switch
        {
            TurnaroundPhase.Deplaning => 0.15,
            TurnaroundPhase.Cleaning  => 0.20,
            TurnaroundPhase.Fueling   => 0.25,
            TurnaroundPhase.Boarding  => 0.40,
            _                         => 1.0
        };
    ...
    private void UpdateFuel(double simDeltaMs)
    {
        ...
        {
            AircraftPhase.Takeoff  => 3.5, AircraftPhase.Climbing => 2.8,
            AircraftPhase.GoAround => 2.8, AircraftPhase.Holding  => 1.5,
            AircraftPhase.Pushback => 0.2, AircraftPhase.Taxiing  => 0.4,
            AircraftPhase.Rollout  => 0.8, AircraftPhase.Diverted => 2.0,
            _                      => 1.2
        };
        double typeMultiplier = State.Type switch
        {
            AircraftType.Small => 0.6, AircraftType.Large => 1.8, _ => 1.0
        };

        State.CurrentFuelPercent = Math.Max(0,
            State.CurrentFuelPercent - (baseBurnRatePerMs * phaseMultiplier * typeMultiplier * simDeltaMs));

        if (State.CurrentFuelPercent <= 10.0 && State.CurrentFuelPercent > 2.0
            && State.Status is not (AircraftStatus.Emergency or AircraftStatus.Diverting))
        {
            DeclareEmergency("Low Fuel (Bingo)");
        }
        else if ((State.CurrentFuelPercent <= 2.0 || State.GoAroundCount >= 3)
                 && State.Status != AircraftStatus.Diverting
                 && State.Phase  != AircraftPhase.Diverted
                 && State.FlightType == FlightType.Arrival
                 && State.Phase is not (AircraftPhase.Rollout or AircraftPhase.Taxiing or AircraftPhase.Landing))
        {
            State.Status          = AircraftStatus.Diverting;
            State.Phase           = AircraftPhase.Diverted;
    ...
    private void AdvancePhase(RunwayController? runway, GateManager gates,
                               WeatherCondition weather, int rvrMeters)
    {
    ...
    private void AdvanceArrivalPhase(RunwayController? runway, GateManager gates,
                                      WeatherCondition weather, int rvrMeters)
    {
        ...
        switch (State.Phase)
        {
            ...
            case AircraftPhase.OnFinal:
                ...
                {
                    AircraftType.Small => 550, AircraftType.Medium => 300, AircraftType.Large => 200, _ => 550
                };
            ...
            case AircraftPhase.Rollout:
                ...
                _currentRoute  = gates.GetArrivalRoute(
                    new SimPoint(600, runwayY), new SimPoint(gateX, gateY));
                break;
    ...
    private void SetPhaseDuration()
    {
        ...
        {
            AircraftType.Small => 1.0, AircraftType.Large => 2.0, _ => 1.5
        };
        double baseMinutes = State.Phase switch
        {
            AircraftPhase.Approaching => 5.0, AircraftPhase.OnFinal  => 2.0,
            AircraftPhase.GoAround    => 3.0, AircraftPhase.Landing  => 0.5,
            AircraftPhase.Rollout     => 1.0, AircraftPhase.Taxiing  => 3.0,
            AircraftPhase.Pushback    => 1.0, AircraftPhase.Holding  =>
                State.FlightType == FlightType.Arrival ? 4.0 : 0.0,
            AircraftPhase.Takeoff  => 0.5, AircraftPhase.Climbing  => 3.0,
            AircraftPhase.Diverted => 4.0, _                       => 1.0
        };
    ...
    private void UpdatePositionAndHeading()
    {
        ...
        if (State.Phase == AircraftPhase.Taxiing && _currentRoute != null && _currentRoute.Count >= 2)
        {
            ...
            for (int i = 0; i < _currentRoute.Count - 1; i++)
            {
                ...
                {
                    double segProgress = Math.Clamp((targetDist - accum) / segDists[i], 0.0, 1.0);
                    double curX        = _currentRoute[i].X +
                                        (_currentRoute[i + 1].X - _currentRoute[i].X) * segProgress;
                    double curY        = _currentRoute[i].Y +
                                        (_currentRoute[i + 1].Y - _currentRoute[i].Y) * segProgress;
        ...
        {
            case AircraftPhase.Approaching:
                (startX, startY, endX, endY) = (2200, runwayY - 200, 1600, runwayY - 180);
                State.Heading = 225; break;
            case AircraftPhase.OnFinal:
                (startX, startY, endX, endY) = (1600, runwayY - 180, 1400, runwayY);
                State.Heading = 225; break;
            case AircraftPhase.GoAround:
                (startX, startY, endX, endY) = (1400, runwayY, 2200, 100);
                State.Heading = 45; break;
            case AircraftPhase.Landing:
            case AircraftPhase.Rollout:
                (startX, startY, endX, endY) = (1400, runwayY, 600, runwayY);
                State.Heading = 270; break;
            case AircraftPhase.Parked:
                (startX, startY, endX, endY) = (gateX, gateY, gateX, gateY);
                State.Heading = 0; break;
            case AircraftPhase.Pushback:
            ...
            case AircraftPhase.Holding:
                ...
                (startX, startY, endX, endY) = (State.Position.X, State.Position.Y,
                                                 State.Position.X, State.Position.Y);
                State.Heading = 90; break;
            case AircraftPhase.Takeoff:
                (startX, startY, endX, endY) = (State.Position.X, State.Position.Y, 1580, State.Position.Y);
                State.Heading = 90; break;
            case AircraftPhase.Climbing:
                (startX, startY, endX, endY) = (1580, runwayY, 2200, 80);
                State.Heading = 45; break;
            case AircraftPhase.Diverted:
                (startX, startY, endX, endY) = (State.Position.X, State.Position.Y, -500, -500);
                State.Heading = 315; break;
            default:
                (startX, startY, endX, endY) = (0, 0, 0, 0); break;
        }
    ...
    private void UpdateAltitudeAndSpeed()
    {
        ...
        {
            State.SpeedKts = State.AssignedSpeedKts ??
                (int)(210 * (State.Type switch
                {
                    AircraftType.Small => 0.8, AircraftType.Large => 1.15, _ => 1.0
                }));
            return;
        }

        double groundY   = 520.0;
        int    defaultAlt = (int)Math.Max(0, (groundY - State.Position.Y) * 7.8);

        State.AltitudeFt = (State.AssignedAltitudeFt.HasValue &&
            State.Phase is AircraftPhase.Approaching or AircraftPhase.Climbing
                        or AircraftPhase.Diverted    or AircraftPhase.GoAround)
            ? State.AssignedAltitudeFt.Value
            : defaultAlt;

        int defaultSpeed = State.Phase switch
        {
            AircraftPhase.Approaching => 180, AircraftPhase.OnFinal  => 140,
            AircraftPhase.GoAround    => 160, AircraftPhase.Landing  => 120,
            AircraftPhase.Rollout     => 60,  AircraftPhase.Pushback => 5,
            AircraftPhase.Taxiing     => 15,  AircraftPhase.Parked   => 0,
            AircraftPhase.Takeoff     => 150, AircraftPhase.Climbing => 200,
            AircraftPhase.Diverted    => 220, _                      => 0
        };
        ...
        State.SpeedKts = (State.AssignedSpeedKts.HasValue &&
            State.Phase is AircraftPhase.Approaching or AircraftPhase.Climbing
                        or AircraftPhase.Diverted    or AircraftPhase.GoAround)
            ? State.AssignedSpeedKts.Value
            : (int)(defaultSpeed * factor);
    }
    ...
    private static double CalculateHeading(SimPoint a, SimPoint b)
    {
        ...
        return (radians * (180.0 / Math.PI) + 90 + 360) % 360;
    }
}